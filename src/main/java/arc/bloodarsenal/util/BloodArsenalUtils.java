...
import static arc.bloodarsenal.registry.Constants.Item.*;
...
public class BloodArsenalUtils
{
    ...
    public static boolean insertItemIntoTile(EntityPlayer player, int amount, TileInventory tile, int slot)
    {
        if (tile.getStackInSlot(slot) != null && player.getHeldItemMainhand() != null)
        {
            ...
            player.getHeldItemMainhand().stackSize -= amount;
    ...
    {
        for (Entry<ItemStackWrapper> entry : drops.entrySet())
        {
    ...
    public static boolean hasFullSet(EntityPlayer player, Class<? extends ItemArmor> armor)
    {
        for (EntityEquipmentSlot slot : EntityEquipmentSlot.values())
        {
            ...
            if (itemStack == null || !itemStack.getItem().getClass().isInstance(armor)) // Sketchiness intensifies
                return false;
        }
    ...
    public static class Profilable
    {
        ...
        {
            if (itemStack != null && itemStack.getItem() instanceof IProfilable)
