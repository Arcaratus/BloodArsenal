...
public class ItemBloodInfusedIronSword extends ItemSword implements IVariantProvider
{
    ...
    {
        if (stack.getItemDamage() > 0 && world.getWorldTime() % ConfigHandler.values.bloodInfusedIronToolsRepairUpdate == 0 && !world.isRemote)
        {
            if (entity instanceof EntityPlayer)
            {
                NetworkHelper.getSoulNetwork((EntityPlayer) entity).syphonAndDamage((EntityPlayer) entity, SoulTicket.item(stack, world, entity, ConfigHandler.values.bloodInfusedIronToolsRepairCost));
    ...
    {
        if (I18n.hasKey("tooltip.bloodarsenal.blood_infused_iron.sword.desc"))
            tooltip.add(TextHelper.localizeEffect("tooltip.bloodarsenal.blood_infused_iron.sword.desc"));
